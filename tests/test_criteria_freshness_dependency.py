...
def _make_successful_reconcile_services():
    fake_client = MagicMock()
    fake_client.file_search_stores.list.return_value = []
    ...
    fake_vec.list_criteria_documents = AsyncMock(return_value=[
        {
            "document_id": "docs/abc",
            "display_name": "abc.pdf",
            "custom_metadata_kv": {
                "type": ("criteria", []),
                "stable_id": ("abc", []),
                "original_title_b64": ("YWJjLnBkZg==", []),
...
@pytest.mark.asyncio
async def test_freshness_dependency_calls_reconcile_when_ttl_expired(
    monkeypatch,
):
    ...
    with patch.object(
        ...
        await cf.ensure_criteria_cache_fresh(db=db)
        await cf.ensure_criteria_cache_fresh(db=db)
    assert run.await_count == 2
    run.assert_awaited_with(db)
...
async def test_freshness_dependency_noop_when_reconcile_disabled(monkeypatch):
    ...
    with patch.object(
        ...
        await cf.ensure_criteria_cache_fresh(db=object())
...
async def test_freshness_dependency_swallows_exceptions(monkeypatch):
    ...
    with patch.object(cf, "_run_reconcile_once", new=boom):
        await cf.ensure_criteria_cache_fresh(db=object())  # raises 안 되어야 함


@pytest.mark.asyncio
async def test_ensure_cache_fresh_uses_injected_session(monkeypatch):
    ...
    def fail_if_default_sessionmaker_is_used():
        raise AssertionError("freshness used app.db.async_session_maker")

    try:
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)

        fake_vec, fake_alias = _make_successful_reconcile_services()

        monkeypatch.setattr(
            "app.db.async_session_maker",
            fail_if_default_sessionmaker_is_used,
        )
        with (
            patch(
            ...
            async with session_factory() as db:
                state = AppStateRepository(db)
                uploaded_map = fake_alias.replace.call_args.args[0]
                assert await state.get(KEY_SYNC_STATE) == "ok"
                assert (
                    await state.get(KEY_LAST_ALIAS_MAP_UPDATED_AT)
                    == uploaded_map.updated_at
                )
    finally:
